...
describe('updaterReleaseMetadata', () => {
  it('builds platform-specific asset URLs', () => {
    expect(getExpectedReleaseAssetUrl('1.2.3', 'darwin', 'arm64')).toBe(
      'https://github.com/lazy-agent/hermit/releases/download/v1.2.3/Hermit-1.2.3-arm64.dmg'
    );
    expect(getExpectedReleaseAssetUrl('1.2.3', 'darwin', 'x64')).toBe(
      'https://github.com/lazy-agent/hermit/releases/download/v1.2.3/Hermit-1.2.3-x64.dmg'
    );
    expect(getExpectedReleaseAssetUrl('1.2.3', 'win32', 'x64')).toBe(
      'https://github.com/lazy-agent/hermit/releases/download/v1.2.3/Hermit.Setup.1.2.3.exe'
    );
    expect(getExpectedReleaseAssetUrl('1.2.3', 'linux', 'x64')).toBe(
      'https://github.com/lazy-agent/hermit/releases/download/v1.2.3/Hermit-1.2.3.AppImage'
    );
  });
...
`;
    ...
    expect(getLatestMacMetadataUrl(version)).toBe(
      `https://github.com/lazy-agent/hermit/releases/download/v${version}/latest-mac.yml`
    );
